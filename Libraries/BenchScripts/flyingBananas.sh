...
then
	...
	do 
		...
		do 
			...
			java ${JAVA_ARGS} A_OP_Benchmark $DataPath $maxBufferedDocs $RAMBufferedSizeMB $numThreads $disableIOThrottle $1 >> ~/IWB/Results/squattingTiger.csv
		done
	...
	for ((e=1;e<=10;e++));
        do
                sync
                sudo sh -c 'echo 3 > /proc/sys/vm/drop_caches'
                java ${JAVA_ARGS} A_OP_Benchmark $DataPath3 $maxBufferedDocs $RAMBufferedSizeMB $nT $disableIOThrottle $1 >> ~/IWB/Results/zippingLlama.csv
        done
	echo 'END OF TEST' >> ~/IWB/Results/zippingLlama.csv
	for ((t=1;t<=10;t++));
        do
                sync
                sudo sh -c 'echo 3 > /proc/sys/vm/drop_caches'
                java ${JAVA_ARGS} A_OP_Benchmark $DataPath2 $maxBufferedDocs $RAMBufferedSizeMB $nT $disableIOThrottle $1 >> ~/IWB/Results/zippingLlama.csv
        done
	echo 'END OF TEST' >> ~/IWB/Results/zippingLlama.csv
	for ((p=1;p<=10;p++));
        do
                sync
                sudo sh -c 'echo 3 > /proc/sys/vm/drop_caches'
                java ${JAVA_ARGS} A_OP_Benchmark $DataPath $maxBufferedDocs $RAMBufferedSizeMB $nT $disableIOThrottle $1 >> ~/IWB/Results/zippingLlama.csv
        done
	echo 'END OF TEST' >> ~/IWB/Results/zippingLlama.csv
fi
